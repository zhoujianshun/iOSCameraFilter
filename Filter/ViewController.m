...
- (IBAction)filterCameraClick:(id)sender {
    [self gotoCamera:^{
        ZJSFilterCameraViewController *vc = [[ZJSFilterCameraViewController alloc] init];
        vc.modalPresentationStyle = UIModalPresentationFullScreen;
        [self presentViewController:vc animated:YES completion:nil];
    }];
}

- (IBAction)customCameraClick:(id)sender {
    [self gotoCamera:^{
        ZJSCustomViewController *vc = [[ZJSCustomViewController alloc] init];
        vc.modalPresentationStyle = UIModalPresentationFullScreen;
        [self presentViewController:vc animated:YES completion:nil];
    }];
}
- (IBAction)customFilterCameraClick:(id)sender {
    [self gotoCamera:^{
        ZJSCustomFilterViewController *vc = [[ZJSCustomFilterViewController alloc] init];
        vc.modalPresentationStyle = UIModalPresentationFullScreen;
        [self presentViewController:vc animated:YES completion:nil];
    }];
}

-(void)gotoCamera:(void(^)(void))block{
