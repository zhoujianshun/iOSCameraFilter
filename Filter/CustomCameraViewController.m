...
@interface CustomCameraViewController ()<AVCaptureAudioDataOutputSampleBufferDelegate, AVCaptureVideoDataOutputSampleBufferDelegate, FilterViewDelegate, ZJSImageFilterSelectViewDelegate>
...
- (void)initSubViews
{
    ...
    [_filterView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(bottomBlackView.mas_top);
    }];
}
...
//    self.filterView.ciImage = [result imageByCroppingToRect:CGRectMake(0, 0, 20, 20)];
//    self.filterView.ciImage = [result imageByClampingToRect:CGRectMake(0, 0, 20, 20)];
    // 添加滤镜
    if (self.filter) {
...
//    }
   ...
   // self.ciOutputImage = [result imageByApplyingTransform:transform];
    transform = CGAffineTransformMakeRotation(-M_PI_2);
    
    if ([[self.cameraDeviceInput device] position] == AVCaptureDevicePositionFront) {// 前置要镜像
        result = [result imageByApplyingOrientation:UIImageOrientationUpMirrored];
//       self.ciOutputImage = [self.ciOutputImage imageByApplyingOrientation:UIImageOrientationUpMirrored];
    }else{
  
    }
    result = [result imageByApplyingTransform:transform];
    //
    self.filterView.ciImage = result;
