...
@interface ZJSMTKImageFilterSelectView ()<UICollectionViewDelegateFlowLayout,UICollectionViewDataSource>

@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) NSMutableArray<ZJSMTKImageFilterSelectItemCellViewModel*> *datas;
...
@implementation ZJSMTKImageFilterSelectView
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ZJSMTKImageFilterSelectItemCellViewModel *baseVM = [self.datas objectAtIndex:indexPath.item];
    
    ZJSMTKImageFilterSelectItemCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kZJSMTKImageFilterSelectItemCellIdentify forIndexPath:indexPath];
    cell.viewModel = (ZJSMTKImageFilterSelectItemCellViewModel*)baseVM;
//    [cell.imageView.mtkView setDrawableSize:CGSizeMake(kZJSImageFilterSelectItemCellHeight*3, kZJSImageFilterSelectItemCellHeight*3)];
    return cell;
}
...
-(void)initDatas{
    ...
    for (int i = 0; i < self.nameArray.count; i ++) {
        ZJSMTKImageFilterSelectItemCellViewModel *vm = [[ZJSMTKImageFilterSelectItemCellViewModel alloc] init];
...
- (void)requestData{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        for (int i = 0; i < self.nameArray.count; i ++) {
            ZJSMTKImageFilterSelectItemCellViewModel *vm = self.datas[i];
            vm.metalDevice = self.metalDevice;
            vm.metalCommandQueue = self.metalCommandQueue;
            vm.ciContext = self.ciContext;
            
            NSString *filterName = self.dict[self.nameArray[i]];
            if (self.ciImage) {
                vm.currentCIImage = [self _filterCIImage: self.ciImage name:filterName];
            }
            vm.filterName = filterName;
...
//        DLog(@"currentIndex:%@",@(currentIndex) );
        if ([self.delegate respondsToSelector:@selector(imageFilterSelectView:selectImage:filterName:)]) {
            [self.delegate imageFilterSelectView:self selectImage:self.datas[currentIndex].currentCIImage filterName:filterName];
        }
    }
...
-(UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[ZJSMTKImageFilterSelectItemCell class] forCellWithReuseIdentifier:kZJSMTKImageFilterSelectItemCellIdentify];
...
-(void)setCiImage:(CIImage *)ciImage{
    ...
    if (self.loaded) {
        for (int i = 0; i < self.nameArray.count; i ++) {
            ...
            NSString *filterName = self.dict[self.nameArray[i]];
            if (_ciImage) {
                CIImage *image = [self _filterCIImage:_ciImage name:filterName];
                dispatch_async(dispatch_get_main_queue(), ^{
                    vm.currentCIImage = image;
                });
            }
